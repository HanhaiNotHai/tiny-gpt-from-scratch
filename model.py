...
def apply_causal_mask(scaled_scores: NDArray, causal_mask: NDArray):
...
def softmax_attention_weights(masked_scores:NDArray):
...
def apply_output_projection(attn_out: NDArray, w_o: NDArray) -> NDArray:
    ...
    return np.einsum('bth,hm->btm', attn_out, w_o)
...
def attention_value_backward(d_attn_out: NDArray, cache: dict[str, NDArray]) -> dict[str, NDArray]:
...
def masked_softmax_backward(d_attn: NDArray, cache: dict[str, NDArray]) -> NDArray:
...
def qk_scores_backward(d_scores: NDArray, cache: dict[str, NDArray]) -> dict[str, NDArray]:
    ...
    return {
        'd_q': np.einsum('bti,bih->bth', d_scores, cache['k']),
...
def qkv_projection_backward(
    ...
    return {
        'dx': sum(
            np.einsum('bth,mh->btm', d, cache[w])
            for d, w in zip((d_q, d_k, d_v), ('w_q', 'w_k', 'w_v'))
        ),
        'dw_q': np.sum(np.einsum('btm,bth->bmh', cache['x'], d_q), axis=0),
        'dw_k': np.sum(np.einsum('btm,bth->bmh', cache['x'], d_k), axis=0),
...
def create_multihead_qkv_projections(d_model: int, scale: float = 0.02):
...
def create_multihead_output_projection(d_model: int, scale: float = 0.02):
...
def reshape_to_heads(x: NDArray, n_heads: int, d_head: int):
...
def get_multihead_n_heads(config: dict[str,int]):
...
def multihead_masked_softmax_scores(scores: NDArray, mask: NDArray):
...
def transpose_heads_to_back(x_heads: NDArray):
    '''move the heads axis back so the result has shape (B, T, n_heads, d_head).'''

    return x_heads.swapaxes(-2, -3)

# Step 127 - get_multihead_output_sequence_length
def get_multihead_output_sequence_length(x_heads_back: NDArray) -> int:
...
def ffn_linear_one_forward(
    x: NDArray, w1: NDArray, b1: NDArray
) -> dict[str, NDArray | dict[str, NDArray]]:
